import{a as kt}from"./api-BJ92196s.js";import{_ as wt,k as N,i as vt,o as Lt,b as y,c as E,d as tt,f as et,g as K,e as rt,s as Mt,h as D,n as Tt,l as M,j as T,G as _t,H as St,I as It,J as Ct,u as Gt}from"./index-DEpp49qz.js";const Nt={class:"page-stack"},Dt={key:0,class:"page-state"},zt={key:2,class:"schedule-poster-section"},Ht={class:"schedule-poster-frame"},jt={class:"schedule-poster-toolbar"},At={__name:"CoursesView",setup(Bt){const z=N(!1),_=N(""),R=N([]),H=N(vt(new Date)),g=e=>String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;"),j=(e,t)=>{const i=String(e??"");return i.length<=t?i:`${i.slice(0,Math.max(0,t-1))}…`},k=e=>{const t=new Date(e);return t.getHours()*60+t.getMinutes()},ot=e=>{const t=Math.floor(e/60);return`${String(t).padStart(2,"0")}:00`},it=e=>`data:image/svg+xml;charset=utf-8,${encodeURIComponent(e)}`,at=()=>{if(!$.value||typeof window>"u"||typeof document>"u")return;const e=document.createElement("iframe");e.setAttribute("aria-hidden","true"),e.style.position="fixed",e.style.width="0",e.style.height="0",e.style.border="0",e.style.right="0",e.style.bottom="0",document.body.appendChild(e);const t=()=>{window.setTimeout(()=>{document.body.contains(e)&&document.body.removeChild(e)},300)},i=e.contentWindow;if(!i){t();return}const a=g(`本周课程表 ${Z.value}`);i.document.open(),i.document.write(`
    <!doctype html>
    <html lang="zh-CN">
      <head>
        <meta charset="UTF-8" />
  ...
  `),i.document.close();const o=i.document.getElementById("poster"),f=()=>{i.focus(),i.print()};if(i.onafterprint=t,o!=null&&o.complete){window.setTimeout(f,120);return}o==null||o.addEventListener("load",()=>{window.setTimeout(f,120)},{once:!0}),o==null||o.addEventListener("error",t,{once:!0})},st=async()=>{z.value=!0,_.value="";try{R.value=await kt.listWeekSchedules(D(H.value))}catch(e){_.value=Tt(e,"课表加载失败")}finally{z.value=!1}},lt=T(()=>(R.value??[]).slice().sort((e,t)=>new Date(e.startTime)-new Date(t.startTime)).map(e=>{const t=String(e.status??"").toUpperCase()==="COMPLETED";return{...e,scheduleId:e.id,studentName:e.studentName??"未命名",subject:e.subject??"正式课",timeRange:Gt(e.startTime,e.endTime),isCompleted:t}})),x=T(()=>{const e=new Map;return lt.value.forEach(t=>{const i=D(t.startTime),a=[i,t.startTime,t.endTime,t.subject].join("|"),o=e.get(a);if(o){o.scheduleIds.push(t.scheduleId),o.studentIds.add(t.studentId??t.scheduleId),o.studentNames.add(t.studentName),t.isCompleted?o.completedScheduleIds.push(t.scheduleId):o.pendingScheduleIds.push(t.scheduleId);return}e.set(a,{groupKey:a,dateKey:i,startTime:t.startTime,endTime:t.endTime,sortTime:new Date(t.startTime).getTime(),timeRange:t.timeRange,subject:t.subject,scheduleIds:[t.scheduleId],completedScheduleIds:t.isCompleted?[t.scheduleId]:[],pendingScheduleIds:t.isCompleted?[]:[t.scheduleId],studentIds:new Set([t.studentId??t.scheduleId]),studentNames:new Set([t.studentName])})}),Array.from(e.values()).map(t=>{const i=Array.from(t.studentNames),a=t.studentIds.size||t.scheduleIds.length,o=t.completedScheduleIds.length,f=t.pendingScheduleIds.length,u=o===t.scheduleIds.length,A=o>0&&f>0;return{groupKey:t.groupKey,dateKey:t.dateKey,startTime:t.startTime,endTime:t.endTime,sortTime:t.sortTime,timeRange:t.timeRange,subject:t.subject,scheduleIds:t.scheduleIds,participantCount:a,participantLabel:`共 ${a} 位学员`,studentNamesLabel:i.join("、"),completedCount:o,isCompleted:u,isPartial:A,sessionNote:a>1?"小组课":"单人课"}}).sort((t,i)=>t.sortTime-i.sortTime)}),S=T(()=>{const e=D(new Date);return St(H.value).map(t=>{const i=D(t),a=t.getDay(),o=x.value.filter(f=>f.dateKey===i);return{dateKey:i,weekday:Ct(t),dateLabel:It(t),dateNumber:`${t.getDate()}`.padStart(2,"0"),monthLabel:`${t.getMonth()+1}月`,isToday:i===e,isWeekend:a===0||a===6,items:o}})}),Z=T(()=>_t(H.value)),$=T(()=>{if(!S.value.length)return"";const e=1600,t=36,i=122,a=12,o=Math.floor((e-t*2-i-a*6)/7),f=162,u=78,A=38,nt=8*60,ct=22*60;let m=nt,I=ct;if(x.value.length){const r=Math.min(...x.value.map(s=>k(s.startTime))),n=Math.max(...x.value.map(s=>k(s.endTime)));m=Math.max(7*60,Math.floor(r/60)*60-60),I=Math.min(23*60,Math.ceil(n/60)*60+60),I-m<8*60&&(I=Math.min(23*60,m+8*60))}const B=Math.max(1,Math.ceil((I-m)/60)),w=84,h=t+f+u,C=B*w,l=h+C+A,p=t+i,v=t+f,dt=new Map(S.value.map((r,n)=>[r.dateKey,n])),V=7*o+6*a,U=e-t*2,Q=f-24,P=v-16,W=C+u+8,G=p-24,O=t+12,F=i-42,X=28,ft=S.value.map((r,n)=>{const s=p+n*(o+a),d=r.isToday?"#dbeafe":r.isWeekend?"#f0f9ff":"#ffffff",c=r.isToday?"#60a5fa":"#dbe7f3",L=r.items.length?`${r.items.length} 节课`:"空档";return`
      <g>
    ...
    `}).join(""),gt=S.value.map((r,n)=>`<rect x="${p+n*(o+a)}" y="${h}" width="${o}" height="${C}" rx="26" fill="${r.isWeekend?"#f8fcff":"#ffffff"}" stroke="#e5edf5" />`).join(""),ht=Array.from({length:B+1},(r,n)=>{const s=h+n*w;return`
    ...
    <g>
      ...
      ${Array.from({length:B},(r,n)=>{const d=h+n*w+8,c=d+X/2,L=G+10,b=p-8;return`
          <g>
            <rect x="${O}" y="${d}" width="${F}" height="${X}" rx="14" fill="rgba(255,255,255,0.92)" stroke="rgba(191,219,254,0.92)" />
            <text x="${O+F/2}" y="${d+18}" text-anchor="middle" font-size="12" font-weight="700" fill="#475569">${g(ot(m+n*60))}</text>
            <circle cx="${G}" cy="${c}" r="5" fill="#ffffff" stroke="#93c5fd" stroke-width="3" />
            <line x1="${L}" y1="${c}" x2="${b}" y2="${c}" stroke="rgba(191,219,254,0.78)" stroke-width="2" stroke-linecap="round" />
          </g>
        `}).join("")}
    </g>
  `,ut=r=>r.isCompleted?{fill:"#f0fdf4",stroke:"#86efac",accent:"#22c55e",badge:"#dcfce7",badgeText:"#15803d"}:r.isPartial?{fill:"#eff6ff",stroke:"#93c5fd",accent:"#3b82f6",badge:"#dbeafe",badgeText:"#1d4ed8"}:{fill:"#fff7ed",stroke:"#fdba74",accent:"#f59e0b",badge:"#ffedd5",badgeText:"#c2410c"},xt=x.value.map(r=>{const n=dt.get(r.dateKey);if(n===void 0)return"";const s=ut(r),d=p+n*(o+a)+10,c=h+(k(r.startTime)-m)/60*w+8,L=o-20,b=Math.max(76,(k(r.endTime)-k(r.startTime))/60*w-12),Y=10,q=d+3,J=b<102,yt=r.isCompleted?"已销课":r.isPartial?`已销 ${r.completedCount}/${r.scheduleIds.length}`:`${r.sessionNote} · ${r.participantLabel}`;return`
      <g filter="url(#cardShadow)">
        <rect x="${d}" y="${c}" width="${L}" height="${b}" rx="20" fill="${s.fill}" stroke="${s.stroke}" />
        <line x1="${q}" y1="${c+Y}" x2="${q}" y2="${c+b-Y}" stroke="${s.accent}" stroke-width="5" stroke-linecap="round" />
        ...
        ${J?"":`<text x="${d+14}" y="${c+80}" font-size="12" fill="#475569">${g(j(r.studentNamesLabel,24))}</text>`}
        ${J?"":`<text x="${d+14}" y="${c+100}" font-size="11" fill="#64748b">${g(r.participantLabel)}</text>`}
        <text x="${d+14}" y="${c+b-18}" font-size="11" fill="${s.badgeText}">${g(j(yt,18))}</text>
      </g>
    `}).join(""),$t=x.value.length?"":`
      <g>
        <rect x="${p+120}" y="${h+110}" width="${V-240}" height="220" rx="36" fill="rgba(255,255,255,0.84)" stroke="#dbe7f3" />
  ...
  `,bt=`
    <svg xmlns="http://www.w3.org/2000/svg" width="${e}" height="${l}" viewBox="0 0 ${e} ${l}">
      ...
      <rect x="${t}" y="${t}" width="${U}" height="${Q}" rx="34" fill="url(#headerGlass)" stroke="rgba(255,255,255,0.88)" />
      <rect x="${t+14}" y="${t+14}" width="${U-28}" height="${Q-28}" rx="28" fill="rgba(255,255,255,0.12)" stroke="rgba(191,219,254,0.38)" />
      <rect x="${t}" y="${P}" width="${i-18}" height="${W}" rx="30" fill="url(#boardGlass)" stroke="rgba(203,213,225,0.82)" />
      <rect x="${t+10}" y="${P+14}" width="${i-38}" height="${W-28}" rx="24" fill="rgba(255,255,255,0.30)" stroke="rgba(255,255,255,0.42)" />
      <rect x="${p-18}" y="${P}" width="${V+36}" height="${W}" rx="34" fill="url(#boardGlass)" stroke="rgba(191,219,254,0.82)" />
      ...
      <text x="${t}" y="${t+120}" font-size="17" fill="#475569" font-weight="700" letter-spacing="0.9" font-family="'Avenir Next', 'DIN Alternate', 'Helvetica Neue', Arial, sans-serif">${g(Z.value)}</text>
      ${ft}
      ${gt}
      ${pt}
      ${ht}
  ...
  `;return it(bt)});return Lt(async()=>{await st()}),(e,t)=>{const i=M("el-alert"),a=M("el-button"),o=M("el-image"),f=M("el-empty"),u=M("el-card");return y(),E("section",Nt,[tt(u,{shadow:"never",class:"schedule-board-card"},{default:et(()=>[z.value?(y(),E("div",Dt,"课表加载中…")):_.value?(y(),K(i,{key:1,title:_.value,type:"error","show-icon":"",closable:!1},null,8,["title"])):(y(),E("div",zt,[rt("div",Ht,[rt("div",jt,[tt(a,{class:"schedule-print-button",disabled:!$.value,onClick:at},{default:et(()=>[...t[0]||(t[0]=[Mt(" 打印课表 ",-1)])]),_:1},8,["disabled"])]),$.value?(y(),K(o,{key:0,src:$.value,"preview-src-list":[$.value],class:"schedule-poster-image",fit:"contain","preview-teleported":"","hide-on-click-modal":""},null,8,["src","preview-src-list"])):(y(),K(f,{key:1,description:"本周暂无排课安排","image-size":72}))])]))]),_:1})])}}},Et=wt(At,[["__scopeId","data-v-089b01f1"]]);export{Et as default};
